...
class PostFilter(filters.FilterSet):
    ...
    def annotate_by_interesting(self, queryset: T) -> T:
        ...
        return queryset.annotate(is_interesting=Exists(following.filter(id=OuterRef('author__profile__id'))))
    ...
    def ordering_filter(self, queryset: T, name: str, value: str) -> T:
        ...
        if self.data.get('is_interesting'):
            return self.annotate_by_interesting(queryset).order_by('-is_interesting', value)
    ...
    def filter_interesting(self, queryset: T, name: str, value: bool) -> T:
        ...
        return self.annotate_by_interesting(queryset).order_by('-is_interesting', '-created_at') # thx to Dan Tyan (this is fix bug with paginate_by)
    ...
    def filter_popular(self, queryset, name: str, value: bool):
        ...
        return self.annotate_by_liked_cnt(queryset).order_by('-liked_cnt', '-created_at')
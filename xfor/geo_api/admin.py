...
class CustomCityAdmin(CityAdmin):
    ...
    list_display = ['alternate_names', 'name_ascii', 'region', 'country', 'subregion', 'timezone']
    list_select_related = ['country', 'region', 'subregion']

class CustomCountryAdmin(CountryAdmin):
    '''Custom Country admin'''

    list_display = ['alternate_names', 'name_ascii', 'code2', 'code3', 'continent', 'phone']
...
to_register = [
    (City, CustomCityAdmin), (SubRegion, CustomSubRegionAdmin),\
    (Region, CustomRegionAdmin), (Country, CustomCountryAdmin)]
